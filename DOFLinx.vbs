...
'   It's by DDH69 and is the version to work with DOFLinx version 7.20
...
'     3) Add FF_DOFLinx=1 below the FF_Init to allow people to enable / disable manually.
'        This flag can be left set as 1, if no link is detected normal sounds will be played anyway - its a manual override.
'     4) Replace the appropriate "PlaySound X,V " statements with "FF_Sound DD,TTTTX,X,V
...
'         TTTT = time on in one hundredths of a second, so 100 is one second, 25 is a quarter of a second
'                -1 is the default time set in the DOFLinx config for that device (usually the way for solenoids)
...
Const DV_RGB=14            ' For Testing RGB existence
Const DV_FN=20             ' Fan
...
Const DV_BE=29             ' Bell
Const DV_C1=30             ' Chime #1
Const DV_C2=31             ' Chime #2
Const DV_C3=32             ' Chime #3
...
Const FL_FL=4              ' Flash the Flasher for number of iterations with the supplied delta between iterations
...
Sub FF_ROM(TheROM)
  if FF_Active = true then
    ...
    DDH_Slots(64,1)=DDH_Slots(64,1)+1
    if DDH_Slots(64,1)>9 then
      DDH_Slots(64,1)=1
    end if
    DDH_Slots(64,0)=12345678910+DDH_Slots(64,1)
  end if
End Sub
...
Sub FF_DOF(TheEvent,TheRef,TheTime)
  ...
  DDH_Slots(65+DOFCycle,1)=DDH_Slots(65+DOFCycle,1)+1
  if DDH_Slots(65+DOFCycle,1)>9 then
    DDH_Slots(65+DOFCycle,1)=1
  end if
  if TheTime = 0 then
    OnOff=0
  else
  ...
  DDH_Slots(65+DOFCycle,0)=(OnOff*1000000000000)+(TheType*100000000000)+(TheRef*10000000)+(TheTime*10)+DDH_Slots(65+DOFCycle,1)
End Sub
...
Sub FF_PUPlayer(TheEvent,TheStr,TheArg)
  ...
  DDH_Slots(37+PUPCycle,1)=DDH_Slots(37+PUPCycle,1)+1
  if DDH_Slots(37+PUPCycle,1)>9 then
    DDH_Slots(37+PUPCycle,1)=1
  end if
  FF_EncColour TheStr,119+(PUPCycle*3)
  DDH_Slots(37+PUPCycle,0)=1000000000000+(TheArg*100)+(TheEvent*10)+DDH_Slots(37+PUPCycle,1)
End Sub